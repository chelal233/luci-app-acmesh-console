...
require_text "acmeshApi.write('core_install'"
require_text "acmeshApi.write('core_upgrade'"
...
install_line="$(grep -n "acmeshApi.write('core_install'" "$PAGE" | cut -d: -f1)"
upgrade_line="$(grep -n "acmeshApi.write('core_upgrade'" "$PAGE" | cut -d: -f1)"
